...
from src.test.py.bazel.bzlmod_test_utils import BazelRegistry


class BzlmodTest(test_base.TestBase):

  def setUp(self):
    test_base.TestBase.setUp(self)
    self.registries_work_dir = tempfile.mkdtemp(dir=self._test_cwd)
    self.main_registry = BazelRegistry(os.path.join(self.registries_work_dir, 'main'))
  ...
  def writeMainProjectFiles(self):
    ...
    self.ScratchFile('A.patch',[
      '--- a/a.cc	2021-09-10 16:44:13.000000000 +0200',
      '+++ b/a.cc	2021-09-10 16:44:41.000000000 +0200',
      '@@ -1,6 +1,6 @@',
      ' #include <stdio.h>',
      ' #include "a.h"',
  ...
  def testSimple(self):
    ...
    exit_code, stdout, stderr = self.RunBazel(['run', '//:main'])
    self.AssertExitCode(exit_code, 0, stderr)
    self.assertIn('main function => A@1.0', '\n'.join(stdout))
  ...
  def testSimpleTransitive(self):
    ...
    exit_code, stdout, stderr = self.RunBazel(['run', '//:main'])
    self.AssertExitCode(exit_code, 0, stderr)
    self.assertIn('main function => B@1.0', '\n'.join(stdout))
    self.assertIn('B@1.0 => A@1.0', '\n'.join(stdout))
  ...
  def testSimpleDiamond(self):
    ...
    exit_code, stdout, stderr = self.RunBazel(['run', '//:main'])
    self.AssertExitCode(exit_code, 0, stderr)
    self.assertIn('main function => A@1.1', '\n'.join(stdout))
    self.assertIn('main function => B@1.0', '\n'.join(stdout))
    self.assertIn('B@1.0 => A@1.1', '\n'.join(stdout))
  ...
  def testSingleVersionOverrideWithPatch(self):
    ...
    exit_code, stdout, stderr = self.RunBazel(['run', '//:main'])
    self.AssertExitCode(exit_code, 0, stderr)
    self.assertIn('main function => A@1.0 (locally patched)', '\n'.join(stdout))
    self.assertIn('main function => B@1.1', '\n'.join(stdout))
    self.assertIn('B@1.1 => A@1.0 (locally patched)', '\n'.join(stdout))


  def testRegistryOverride(self):
    self.writeMainProjectFiles()
    another_registry = BazelRegistry(os.path.join(self.registries_work_dir, 'another'), ' from another registry')
    ...
    exit_code, stdout, stderr = self.RunBazel(['run', '//:main'])
    self.AssertExitCode(exit_code, 0, stderr)
    self.assertIn('main function => A@1.0 from another registry', '\n'.join(stdout))
    self.assertIn('main function => B@1.0', '\n'.join(stdout))
    self.assertIn('B@1.0 => A@1.0 from another registry', '\n'.join(stdout))
  ...
  def testGitOverride(self):
    ...
    self.RunProgram(["git", "init", "-b", "main"], cwd=src_a_1_0)
    self.RunProgram(["git", "add", "./"], cwd=src_a_1_0)
    self.RunProgram(["git", "commit", "-m", "Initial commit."], cwd=src_a_1_0)
    _, stdout, _ = self.RunProgram(["git", "rev-parse", "HEAD"], cwd=src_a_1_0)
  ...
  def testRemotePatchForBazelDep(self):
    ...
    self.main_registry.createCcModule('A', '1.1-1', patches=[patch_file], patch_strip=1)
    self.ScratchFile('WORKSPACE')
    self.ScratchFile('MODULE.bazel', [
    ...
    exit_code, stdout, stderr = self.RunBazel(['run', '//:main'])
    self.AssertExitCode(exit_code, 0, stderr)
    self.assertIn('main function => A@1.1-1 (remotely patched)', '\n'.join(stdout))

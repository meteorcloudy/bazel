...
class BazelRegistry:
  ...
  def generateCcSource(self, name, version, deps = None, repo_names = None):
    ...
    2. The hello_<lib_name> function calls the same function of its dependencies.
    3. The hello_<lib_name> function prints "<caller name> => <lib_name@version>".
    """
    ...
    scratchFile(src_dir.joinpath('MODULE.bazel'),[
        'module(',
        '  name = "%s",' % name,
        '  version = "%s",' % version,
        '  compatibility_level = 1,',
        ')',
    ] + ['bazel_dep(name = "%s", version = "%s"%s)' % (dep, version, calc_repo_name_str(dep)) for dep, version in deps.items()])

    scratchFile(src_dir.joinpath(name.lower() + '.h'),[
        '#ifndef %s_H' % name.upper(),
        '#define %s_H' % name.upper(),
        '#include <string>',
        'void hello_%s(std::string caller);' % name.lower(),
        '#endif',
    ])
    scratchFile(src_dir.joinpath(name.lower() + '.cc'),[
        '#include <stdio.h>',
        ...
        'void hello_%s(std::string caller) {' % name.lower(),
        '    std::string lib_name = "%s@%s%s";' % (name, version, self.registry_suffix),
    ...
    scratchFile(src_dir.joinpath('BUILD'),[
        ...
        '  deps = ["%s"],' % ('", "'.join(['@%s//:lib_%s' % (repo_names[dep], dep.lower()) for dep in deps])),
    ] if deps else []) + [
        ')',
    ])
    return src_dir
  ...
  def createCcModule(self, name, version, deps = None, repo_names = None, patches = None, patch_strip = 0):

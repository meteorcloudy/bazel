...
function get_release_name() {
  # Match release-X.X.X-rcY or X.X.X and return X.X.X
  git_get_branch | grep -Po "(?<=release-)([0-9]|\.)*(?=rc)" || git_get_branch | grep -Po "^([0-9]|\.)*$" || true
}
...
# Extract the full release name from the branch names
